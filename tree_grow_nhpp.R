...
tree.grow = function(i.node, Data_input, X.B_input, Z.B_input, Terminal.list_input){
  ...
  if (Initial.node_input[nrow(Initial.node_input),4]==0){
    ...
    obj.list = lapply(1:length(x.sample), tree.sub.fun2, Data_id_sub2=Data_id_input,
                      X.B_sub2=X.B_input, Z.B_sub2=Z.B_input, data.B_sub2=Data_input, X.sample = x.sample)
    obj = do.call(cbind,obj.list)
    print(obj)
    for (i in 1:ncol(obj)){
      case = which(obj[,i]==Inf)
      if (length(case)>0){obj[case,i]=-1}
    }
